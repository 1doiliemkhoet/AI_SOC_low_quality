...
async def receive_wazuh_alert(alert: WazuhAlert):
    ...
    try:
        ...
        if alert.rule.level >= app.state.settings.rag_severity_threshold:
            ...
            # Extract MITRE techniques already provided by Wazuh
            ...
            # ---------------------------------------------------------
            # RAG ENRICHMENT WITH FULL EVIDENCE
            # ---------------------------------------------------------
        ...
        enriched_alert = EnrichedAlert(
            ...
            mitre_techniques=(
                alert.rule.mitre.get("id", [])
                if alert.rule.mitre
                else []
            ),
            source_ip=(
                alert.data.srcip
                if alert.data and alert.data.srcip
                else (
                    alert.agent.ip
                    if alert.agent
                else None
            ...
            dest_ip=(
                alert.data.dstip
                if alert.data and alert.data.dstip
                else None
            ),
