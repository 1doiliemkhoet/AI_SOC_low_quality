...
class WazuhClient:
    ...
    async def get_alerts(
        ...
        """
        Fetch alerts from Wazuh Indexer.

        Args:
            min_level: Minimum rule level (0-15).
            limit: Maximum number of alerts to return.
            offset: Offset for pagination.
            time_range: Time range such as "1h", "24h", "7d".
        ...
        # Wazuh Indexer / OpenSearch endpoint
        alerts_url = f"{self.settings.wazuh_indexer_url}/wazuh-alerts-*/_search"
        ...
        try:
            async with httpx.AsyncClient(
                verify=self.settings.indexer_verify_ssl
            ) as client:

                response = await client.post(
                    alerts_url,
                    auth=(
                        self.settings.indexer_username,
                        self.settings.indexer_password
                    ),
                    json=query,
                    timeout=self.settings.wazuh_api_timeout
                )
                ...
                for hit in hits:
                    ...
                    # Keep Elasticsearch/OpenSearch document ID
